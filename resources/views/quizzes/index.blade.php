...
@section('title', 'Quizzes - Prism AI')
    ...
    <div class="max-w-5xl mx-auto">
        <div class="flex items-center justify-between mb-8">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Test your knowledge with AI-generated quizzes</p>
            </div>
        </div>
        ...
        @if($quizzes->isEmpty())
            <div class="text-center py-16">
                ...
                <h3 class="text-lg font-medium text-gray-600 mb-1">No quizzes yet</h3>
                <p class="text-sm text-gray-400">Select "Generate Quiz" from the dashboard to create your first quiz.</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                @foreach($quizzes as $quiz)
                    <div class="bg-white border border-gray-200 rounded-2xl p-5 hover:shadow-md transition-shadow">
                        ...
                        <p class="text-sm text-gray-500 mb-3">{{ $quiz->total_questions }} questions</p>

                        @if($quiz->attempts->isNotEmpty())
                            <div class="flex items-center gap-1.5 mb-3">
                                <span class="text-xs font-medium text-green-600 bg-green-50 px-2 py-0.5 rounded-full">
                                    Best: {{ $quiz->attempts->first()->score }}/{{ $quiz->attempts->first()->total_questions }}
                                </span>
                            </div>
                        @endif

                        <div class="flex items-center justify-between">
                            <span class="text-xs text-gray-400">{{ $quiz->created_at->diffForHumans() }}</span>
                            <a href="{{ route('quizzes.show', $quiz) }}" class="text-sm font-medium text-purple-600 hover:text-purple-700 transition-colors">
                                Take Quiz &rarr;
                            </a>
                        </div>
                    </div>
